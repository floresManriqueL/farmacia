...
function redirec(){
    if(document.getElementById('correo').value == ""){
      str = "correo";
              ...
              cor.style.cssText = 'color: rgb(255, 0, 0);';
              correo.style.cssText = 'border-color: rgb(255, 0, 0);';
              Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: te,
       ...
       document.getElementById('bandera').value = "add1";
       document.farmaci.submit();
    }
}
    jQuery(function($){
        ...
        <div class="row">
          ...
          <input type="hidden" name="baccion" id="baccion" value="">
                                ...
                                <div class="form-group">
                                 <label id="cor" class="control-label"><i class="fa fa-envelope" aria-hidden="true"></i> CORREO</label>
            <input class="form-control" type="text" id="correo" name="correo" placeholder="email">
                                </div>
                               <div class="form-group">
            <div class="utility">
            ...
            <button class="btn btn-primary btn-block" onclick="redirec()"><i class="fa fa-check-square-o"></i> Recupera Contra con Correo</button>                                
                               </div>
   ...
   // $dui=(isset($_REQUEST["dui"]))?$_REQUEST["dui"]:"";
    ...
    if($bandera=="add"){
      echo $correo;
          ...
          echo "location.href ='gmail.php'";
          echo "</script>";
        require_once "conexion/DB_mysql.php";  
        $Conexion_ID=new DB_mysql();
        $Conexion_ID= $Conexion_ID->getConexion();
        $result=$Conexion_ID->query("SELECT nombre as nombre, dui as dui, correo as correo, telefono as telefono FROM empleado WHERE correo = '$correo'");
        if($result){
           echo "<script type='text/javascript'>"; 
         echo " Swal.fire({
  title: 'SyS-Farmacia',
